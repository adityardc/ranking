...
<div class="row">
    <div class="col-lg-6">
        <div class="card-box ribbon-box">
            <div class="ribbon ribbon-blue float-left"><i class="mdi mdi-access-point mr-1"></i> Form Ubah Role Pengguna</div>
            <div class="ribbon-content">
                <form role="form" id="frmBagian" method="POST" action="{{ route('role.update', $data->id) }}">
                    @csrf
                    {{ method_field('PUT') }}
                    ...
                    <div class="form-group">
                        ...
                        <select class="select2-multiple form-control" name="permission[]" multiple="multiple" required>
                            @foreach ($permission as $val)
                                <option value="{{ $val->id }}" {{ (in_array($val->id, $rolePermissions)) ? "selected='selected'" : "" }}>{{ $val->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
