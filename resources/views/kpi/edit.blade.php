...
<div class="row">
    <div class="col-lg-6">
        <div class="card-box ribbon-box">
            <div class="ribbon ribbon-blue float-left"><i class="mdi mdi-access-point mr-1"></i> Form Ubah Indikator KPI</div>
            <div class="ribbon-content">
                <form role="form" id="frmBagian" method="POST" action="{{ route('kpi.update', $kpi->id) }}">
                    @csrf
                    {{ method_field('PUT') }}

                    <div class="form-group has-error">
                        <label for="exampleInputUsername2">Nama KPI <code>*</code></label>
                        <textarea name="nama_kpi" id="nama_kpi" cols="30" rows="5" class="form-control" required autofocus>{{ $kpi->nama_kpi }}</textarea>
                    </div>
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select name="satuan_id" id="satuan_id" class="form-control select2-multiple" required>
                                    ...
                                    @foreach ($sat as $itemSat)
                                        <option value="{{ $itemSat->id }}" {{ ($itemSat->id == $kpi->satuan_id) ? "selected='selected'" : "" }}>{{ $itemSat->kode_satuan }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="kategori_id" id="kategori_id" class="form-control select2-multiple" required>
                                    ...
                                    @foreach ($kat as $itemKat)
                                        <option value="{{ $itemKat->id }}" {{ ($itemKat->id == $kpi->kategori_id) ? "selected='selected'" : "" }}>{{ $itemKat->nama_kategori }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="icon" id="icon" class="form-control" required>
                            ...
                            <option value="0" {{ ($kpi->icon == 0) ? "selected='selected'" : "" }}>SEMAKIN NAIK SEMAKIN BAIK</option>
                            <option value="1" {{ ($kpi->icon == 1) ? "selected='selected'" : "" }}>SEMAKIN TURUN SEMAKIN BAIK</option>
                        </select>
                    </div>
                    <div class="form-group">
